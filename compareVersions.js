...
function changeToInteger (string) {
    const arrayFromString = string.split('.');

    const arrayOfNumbers = arrayFromString.map(element => {
    ...
    for(let i = 0; i < arrayOfNumbers.length; i++) {
        let x = (arrayOfNumbers[i] / n).toFixed(i);
        result += parseFloat(x);
        n *= 10;
    }
    return result;
...
function compareVersions (version1, version2) {
    const integerOfV1 = changeToInteger(version1);
    const integerOfV2 = changeToInteger(version2);


    if(integerOfV1 >= integerOfV2) {
        return true;
    }
    if(integerOfV1 < integerOfV2) {
        return false;
